...
extern int IX0, IXD, IY0, IYD;
      ...
      // int IX0=20, IY0=600, IXD=400, IYD=400;
       int Nsq=0;
/****************************************  POLE  ***********************/
/* Построение изолиний ( f(x,y)=c ) параметр c = от Ha до Hb с шагом HL*/
/* Функция в Zz,   x:   0 < x < Lx      0 < y < Ly                     */
/* Параметр Z - коэффициент сжатия по вертикали                        */
/* Остальные параметры передаются как глобальные (extern вверху)       */
/************************************************************************/
